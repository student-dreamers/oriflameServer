...
import { TEST_EXISTING_PRODUCT_UUID, TEST_NONEXISTING_UUID, TEST_EXISTING_PRODUCT_EAN } from '../src/config';
...
export default describe('Product route', () => {
    ...
    it('should get existing product by uuid', () =>
        request(app)
            .get(`/products/${TEST_EXISTING_PRODUCT_UUID}`)
            .expect(200));

    it('should get correct existing product by ean', () =>
        request(app)
            .get(`/products/${TEST_EXISTING_PRODUCT_EAN}`)
            .expect(200)
            .expect(({ body }) => {
